...
def average_precision(pos_distances, neg_distances, show_plot=False):
    ...
    print("len of distances:", len(distances))
    ...
    # Calculate precision
    # print("Calculating Precision: ", matches, np.cumsum(matches), np.arange(1,len(matches) + 1))
    ...
    # print("Average Precision inside: ", average_precision)
    ...
    if show_plot:
        ...
        plt.savefig("RecallvPrecision.png")
...
def generate_matches_array(labels):
    ...
    for n in range(N):
        cur_label = labels[n]
        matches[cur_matches_i:cur_matches_i + (N - n) - 1] = np.asarray(labels[n + 1:]) == cur_label # https://stackoverflow.com/questions/40659212/futurewarning-elementwise-comparison-failed-returning-scalar-but-in-the-futur
        cur_matches_i += N - n - 1
    # print("matches", matches)
    # print("labels", labels)
    # print("len of labels", len(matches))
    return matches
