...
def iterate_data(model, sess, compute_loss, mode, verbose, num_steps=None):
	...
	for worker_id in range(model.num_gpus):
		# test = tf.get_default_graph().get_tensor_by_name('ForwardPass/ds2_encoder/conv2/kernel:0')
		cur_fetches = [
				model.get_data_layer(worker_id).input_tensors,
				model.get_output_tensors(worker_id),
				model.get_interest_tensors(worker_id),
				# test
		]
		print(cur_fetches)
		if compute_loss:
	...
	while True:
		...
		if size_defined:
			...
			# print(fetches_to_run)
			fetches_vals = sess.run(fetches_to_run)
			# opers = sess.graph.get_operations()
			# print(opers.name)
			# for i in opers:
			#   print(i.name)
				# sys.exit()
			# print(fetches_vals[0][2][0])
			# print(fetches_vals[0][2][0].shape)
			# sys.exit()
			# print(np.asarray(fetches_vals[0]))
			# print(np.asarray(fetches_vals[0][2]).shape)
			# print("shape: ", fetches_vals[0][0]["source_tensors"][0].shape)
			# sys.exit() 
			# print(fetches_vals) 
			if len(fetches_vals)!=0 :
				# print(fetches_vals[0][-2])
				# print(fetches_vals[0][-2].shape)
				# analysis_folder = "/ssd_scratch/cvit/kiran/embeddings_analysis"
				# model_folder="model.ckpt-0" #model_path
				# save_location = "embeddings1/emb1_"
				# np.save(os.path.join(analysis_folder, model_folder, save_location) + str(counter), fetches_vals[0][-2])
				np.save("/GeneralizedKWS/testing_output/embeddings/embeddings1/emb1_" + str(counter), fetches_vals[0][-2])
				counter += 1
...
def deco_print(line, offset=0, start="*** ", end='\n'):
	if six.PY2:
		print((start + " " * offset + line).encode('utf-8'), end=end, flush=True )
	else:
		print(start + " " * offset + line, end=end, flush=True)
...
def check_logdir(args, base_config, restore_best_checkpoint=False):
	...
	try:
		...
		if args.mode == 'train' or args.mode == 'train_eval':
			...
			if base_config["specific_checkpoint"] != "":
				checkpoint = base_config["specific_checkpoint"]
				return checkpoint
			if os.path.isdir(logdir) and os.listdir(logdir) != []:
