...
def train(train_model, eval_model=None, debug_port=None, custom_hooks=None):
  ...
  if master_worker:
    ...
    # if checkpoint_dir == None or checkpoint_dir == "" :
    #   checkpoint_dir = "/ssd_scratch/cvit/kiran/small_checkpoint"
  else:
    checkpoint_dir = None
    load_model_dir = None

  print(" checkpoint_dir: ", checkpoint_dir, " load_model_dir: ", load_model_dir)
  # checkpoint_dir  ="/ssd_scratch/cvit/kiran/OpenSeq"
  # print("Saves here: ", checkpoint_dir)
  ...
  if restoring:
    ...
    for var in tf.global_variables():
      ...
      # print(var_name, var)
      if var_name == "ForwardPass/ds2_encoder/cudnn_gru/opaque_kernel":
        vars_to_load.append(var)
        print("added: ForwardPass/ds2_encoder/cudnn_gru/opaque_kernel")
    ...
    print("vars in checkpoint ", len(vars_in_checkpoint), " current vars length: ",len(vars_to_load))
    # sys.exit()
  ...
  fetches = [train_model.train_op, train_model.triplet_loss, train_model.loss]
  # fetches = [train_model.triplet_loss]
  # fetches = [train_model.get_data_layer().testing_var]
  try:
    total_objects = 0.0
  ...
  # starting training
  # checkpoint_dir  ="/ssd_scratch/cvit/kiran/testing"
  print("Saves here: ", checkpoint_dir)
  ...
  while True:
    ...
    try:
      ...
      if step % iter_size == 0:
        if step >= bench_start:
          num_bench_updates += 1
        # print(fetches, feed_dict)
        fetches_vals = sess.run(fetches, feed_dict)
        # print(fetches_vals)
        # exit(0)
        print("Triplet loss: {}, Total loss: {} ".format(fetches_vals[1], fetches_vals[2]))
        # print("Distances: ", fetches_vals[3], fetches_vals[4])
        # CHECKING:
        # pd = fetches_vals[3]
        # nd = fetches_vals[4]
        # print("Pos", pd.shape)
        ...
        # for i in pd:
        #   print(i)
        # print("Positive Pairs: ", np.count_nonzero(pd)/2)
        # print("Negative Pairs: ", np.count_nonzero(nd)/2)
        # print("labels: ", fetches_vals[5])
        # exit(0)
      else:
