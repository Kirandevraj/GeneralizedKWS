...
class Model:
  ...
  def __init__(self, params, mode="train", hvd=None):
    ...
    * **larc_params** --- dictionary with parameters for LARC (or LARS)
    ...
    if self.on_horovod:
      ...
      self._outputs1 = [None] * self.num_gpus
      self._outputs2 = [None] * self.num_gpus
      self._outputs3 = [None] * self.num_gpus
      self._cnn_output1 = [None] * self.num_gpus
      self._cnn_output2 = [None] * self.num_gpus
      self._cnn_output3 = [None] * self.num_gpus
      self._rnn_output = [None] * self.num_gpus
      self._rep_loss = [None] * self.num_gpus
    ...
    self.ctc_weight = self._params["ctc_weight"]
    self.triplet_weight = self._params["triplet_weight"]
  ...
  def compile(self, force_var_reuse=False, checkpoint=None):
    ...
    if not self.on_horovod:  # not using Horovod
      ...
      for gpu_cnt, gpu_id in enumerate(self._gpu_ids):
        with tf.device("/gpu:{}".format(gpu_id)), tf.variable_scope(
          ...
          print(input_tensors)
          input_tensors1 = {'source_tensors': [input_tensors['source_tensors'][0], input_tensors['source_tensors'][1]], \
                            'target_tensors': input_tensors['target_tensors'], 'label': input_tensors['label']}
          # input_tensors2 = {'source_tensors': [input_tensors['source_tensors'][2], input_tensors['source_tensors'][3]], \
          #                   'target_tensors': input_tensors['target_tensors']}
          # input_tensors3 = {'source_tensors': [input_tensors['source_tensors'][4], input_tensors['source_tensors'][5]], \
          #                   'target_tensors': input_tensors['target_tensors']}
          self.input_tensors1 = input_tensors1
          if self.params.get("use_trt", False):
            # Build TF-TRT graph
            loss, self._outputs[gpu_cnt], self._cnn_output[gpu_cnt] = self.build_trt_forward_pass_graph(
                input_tensors1,
                gpu_id=gpu_cnt,
                checkpoint=checkpoint
            )
            loss, self._outputs1[gpu_cnt], self._cnn_output1[gpu_cnt] = self.build_trt_forward_pass_graph(
                input_tensors2,
                gpu_id=gpu_cnt,
                checkpoint=checkpoint
            )
            ...
            loss1, self._outputs1[gpu_cnt], self._cnn_output1[gpu_cnt] = self._build_forward_pass_graph(
                input_tensors1,
                gpu_id=gpu_cnt
            )
          self._rep_loss[gpu_cnt] = self.loss_computator._compute_triplet_loss(self._cnn_output1[gpu_cnt], input_tensors['label'])

          if self._outputs1[gpu_cnt] is not None and \
             not isinstance(self._outputs1[gpu_cnt], list):
            raise ValueError('Decoder outputs have to be either None or list')
          if self._mode == "train" or self._mode == "eval":
            #alpha1 = 0.1 and alpha2 = 0.9
            losses.append(self.ctc_weight * loss1 + self.triplet_weight * self._rep_loss[gpu_cnt])
            triplet_for_graph.append(self._rep_loss[gpu_cnt])
            ctc_for_graph.append(loss1)
      
      
      # end of for gpu_ind loop
      if self._mode == "train":
        self.loss = tf.reduce_mean(losses)
        self.triplet_loss = tf.reduce_mean(triplet_for_graph)
      if self._mode == "eval":
      ...
      with tf.device("/gpu:0"), tf.variable_scope(
        ...
        if self.params.get("use_trt", False):
          # Build TF-TRT graph
          all_loss, self._output, self._cnn_output = self.build_trt_forward_pass_graph(
              input_tensors,
              gpu_id=0,
              checkpoint=checkpoint
          )
          ...
          # Build regular TF graph
          all_loss, self._output, self._cnn_output = self._build_forward_pass_graph(
              input_tensors,
              gpu_id=0
          )
    ...
    if self._mode == "train":
      ...
      # print("freeze: ", freeze_variables_regex)
      ...
      self.train_op = optimize_loss(
          ...
          model=self,
          # mse=tf.reduce_mean(mse_for_graph),
          ctc=tf.reduce_mean(ctc_for_graph),
          triplet = tf.reduce_mean(triplet_for_graph)
      )
  ...
  @abc.abstractmethod
  def _build_forward_pass_graph(self, input_tensors, gpu_id=0, reuse=False):
  ...
  def get_output_tensors(self, worker_id=0):
    ...
    if self.on_horovod:
      ...
      return self._outputs1[worker_id]

  def get_interest_tensors(self, worker_id=0):
    if self.on_horovod:
      return self._cnn_output
    else:
      return self._cnn_output1[worker_id]
