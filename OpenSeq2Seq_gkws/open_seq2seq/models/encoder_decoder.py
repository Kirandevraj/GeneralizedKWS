...
class EncoderDecoderModel(Model):
  ...
  def _build_forward_pass_graph(self, input_tensors, gpu_id=0, reuse=False):
    ...
    with tf.variable_scope("ForwardPass"):
      ...
      encoder_output = self.encoder.encode(input_dict=encoder_input, reuse=reuse)

      decoder_input = {"encoder_output": encoder_output}
      if self.mode == "train" or self.mode == "eval":
        decoder_input['target_tensors'] = target_tensors
      decoder_output = self.decoder.decode(input_dict=decoder_input, reuse=reuse)
  ...
  @property
  def classification_decoder(self):
    '''Only for classification of google speech commands dataset'''
    return self._classification_decoder
