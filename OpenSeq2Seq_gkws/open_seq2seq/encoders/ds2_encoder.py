...
class DeepSpeech2Encoder(Encoder):
  ...
  def _encode(self, input_dict, reuse=False):
      ...
      if self.params['use_cudnn_rnn']:
        ...
        if reuse is False:
          if rnn_type == "cudnn_gru" or rnn_type == "gru":
            # pylint: disable=no-member
            self._rnn_block = tf.contrib.cudnn_rnn.CudnnGRU(
            ...
            # pylint: disable=no-member
            self._rnn_block = tf.contrib.cudnn_rnn.CudnnLSTM(
                num_layers=num_rnn_layers,
                num_units=rnn_cell_dim,
                direction=direction,
                dropout=1.0 - dropout_keep_prob,
          ...
          top_layer, state = self._rnn_block(rnn_input)
        else:
          top_layer, state = self._rnn_block(rnn_input)
